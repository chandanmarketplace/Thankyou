...
const ImageWithText = (props) => {
  ...
  const inputChanged = (value) => {
    updateConfig(type,{...dataset, redirect_url: value } )
  };

  const bgColorChanged = (value) => {
    updateConfig(type,{...dataset, bgColor: value } )
  };

  const borderRadiusChanged =(value) => {
    updateConfig(type,{...dataset, borderRadius: value } )
  };

  const handleImageChangEvent = (url) => {
    updateConfig(type,{...dataset, img_url: url } )
  }

  const handleImageAlignmentEvent = (alignment) => {
    updateConfig(type,{...dataset, img_alignment: alignment } )
  }

  const handleImageShapeEvent = (img_layout) => {
    updateConfig(type,{...dataset, img_layout: img_layout } )
  }
        ...
        <div className="panel-body cust-min-height bg-white">
          ...
          <div className="form-group mx-3 my-3 bg-white text-start">
            <div className="select-img mb-3">
              ...
              <div
                id="empty-image-2463415"
                className="empty-img-container margin-bot-20px"
              >
                <img
                  id="eee-img-default"
                  className="img-fluid"
                ...
                <button
                  onClick={(e) => {
                    ...
                    document.querySelector(
                      ".select-image.slide-to"
                    ).style.display = "none";
                    document.querySelector(".btn-group-ss").style.display =
                      "block";
                  }}
                  className="Polaris-Button select-image slide-to selectImgFrmList"
                  type="button"
                  name="button"
                >
                  Select image
                </button>
                <div className="eee-widget">
                  <Widget
                    onChange={(e) => {
                      let url = document.querySelector(
                        "div#empty-image-2463415 input"
                      ).value;
                      handleImageChangEvent(url);
                      // console.log(url);
                      // setValue({
                      //   ...value,
                      //   img_url: url,
                    ...
                    publicKey="93be7c507b15f0355f7b"
                  />
                </div>
              </div>
              <div
                className="btn-group-ss bg-white col-12 row m-0"
                style={{ border: "1px solid #c4cdd5", borderTop: "none" }}
              >
                <div className="d-flex p-0">
                  ...
                  <button
                    ...
                    className="btn btn-default btn-change slide-to selectImgFrmList col-6"
                    style={{ borderRight: "1px solid #c4cdd5" }}
                    type="button"
                  >
                    Change
                  </button>{" "}
                  <button
                    onClick={(e) => {
                      ...
                      handleImageChangEvent("");
                    }}
                    className="btn btn-default btn-remove remove-image col-6"
            ...
            <div className="select-ia">
              ...
              <select
                onChange={(e) => {
                  handleImageAlignmentEvent(e.target.value);
                }}
                value={value.img_alignment}
            ...
            <div className="select-il">
              ...
              <select
                id="eee"
                onChange={(e) => {
                  handleImageShapeEvent(e.target.value)
                  // setValue({
                  //   ...value,
                  //   img_layout: e.target.value,
                  // });
                  console.log(value);
                }}
                value={value.img_layout}
                className="form-select form-select-sm mb-3 "
              >
            ...
            <p style={{ fontSize: "13px" }} className="text-muted">
              https://developement-store-test.myshopify.com
            </p>
            ...
            <div className="input-group mb-1">
              <input type="color" id="favcolor" name="favcolor" value={value?.bgColor} onChange={(e) => {
                bgColorChanged(e?.target?.value)
              }} />
            ...
            <div className="hfs d-flex flex-column align-item-center gap-3 mb-3">
                ...
                <input type="range" min="0" max="64" value={value?.borderRadius || 0} onChange={(e) => {
                borderRadiusChanged(e?.target?.value)
              }}  step="1"/>
            </div>


            <div className="select-ft">
              <label
                htmlFor="image_alignment2463415"
                className="control-label mb-1 fs-6"
              >
